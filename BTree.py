...
class Arbol_B:  # Clase de arboles B
    ...
    def bTreeSearch(self, nodo, k):
        ...
        if i <= x.n and k == x.keys[i]:  # Si el ciclo while se ucmple
            ...
            if x.hoja == 1:
                return None  # No existe, y se regresa un None
            else:  #
                return self.bTreeSearch(x.hijos[i],
                                        k)  # Se llama el metodo de manera recursiva con el nodo hijo en la posicion i
...
if __name__ == '__main__':
    ...
    print(
        f'ROOT-- n: {PinoB.root.n}, keys: {PinoB.root.keys}, leaf: {PinoB.root.hoja}, HIJOS- [0]: {PinoB.root.hijos[0].keys}, [1]: {PinoB.root.hijos[1].keys}')

    # Se vuelven a agregar nuevos valores al array data y de la misma forma se agregararan al arbol
    data = [81, 82, 100, 83, 84]
    j = 1
    ...
    for i in data:  # En el ciclo for se le iran agregando los valores de data junto a un mensaje
        ...
        if i != 83 and i != 84:  # Cuando se agregan valores distintos al ultimo se mostraran la raiz y sus dos hijos
            print(
    ...
    (nodo, posicion) = PinoB.bTreeSearch(PinoB.root, 84)
    print(f'Encontrado el valor 84 en el nodo: {nodo.keys} en la posicion {posicion}')
...
# ULTIMA MODIFICACION 9 DE NOVIEMBRE DEL 2021 A LAS 11:50 PM, TIJUANA BAJA CALIFORNIA